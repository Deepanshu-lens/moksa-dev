...
<div class="block sm:hidden">
  ...
  <div class="flex flex-col w-full bg-[#f5f6f7] dark:bg-neutral-900 z-10 relative h-[100vh]">
    <div class="top-config w-full">
      <button class="flex items-center justify-start text-black/[.7] pt-2">
        <ChevronLeft class="h-[30px] w-[30px] dark:text-white" />
        <p class="text-lg font-semibold dark:text-white">Gallery</p>
      </button>

      <div class="flex items-center justify-between p-2 ">
        <h3 class="text-lg font-semibold dark:text-white">Add new Person:</h3>
        <RegisterMobileDialog>
          <button
            class="flex items-center justify-center gap-2 cursor-pointer relative hover:text-primary text-xl dark:text-white"
          >
            <ScanFace size={18} />
            Register
          </button>
