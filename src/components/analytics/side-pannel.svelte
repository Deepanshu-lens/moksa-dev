<script lang="ts">
  ...
  let theftandcamera = writable([]);
  let theftData = writable([]);
  let usersData = writable([]);

  let aisleData = writable([]);
  let busyness = writable([]);
  ...
  $: {
    if ($analyticsData) {
      allStores.set($analyticsData?.stores?.data?.data);
      allStoresData.set($analyticsData?.allstoreData?.data);
      theftandcamera.set($analyticsData?.theftandcamera?.data?.data);
      theftData.set($analyticsData?.theftData);
      usersData.set($analyticsData?.usersData);
      aisleData.set($analyticsData?.aisleData?.data?.data);
      busyness.set($analyticsData?.busyness?.data);
      efficiency.set($analyticsData?.efficiency?.data);
