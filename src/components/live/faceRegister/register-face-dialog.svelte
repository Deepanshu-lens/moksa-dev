<script lang="ts">
  ...
  let addingPerson = writable<boolean>(false);
  ...
  async function capturePhoto() {
    ...
    if (context) {
      ...
      if (!result.ok) {
        ...
        console.error(error);
        toast.error(error || "Something went wrong. Please try another");
        loading = false;
        return;
      }
      await addUserLogs(
...
<Dialog.Root bind:open={dialogOpen}>
  ...
  <Dialog.Content
    class="sm:max-w-[720px] xl:scale-95 2xl:scale-100 max-h-[95%] sm:max-h-auto overflow-y-scroll overflow-x-clip hide-scrollbar"
  >
    <Dialog.Header>
