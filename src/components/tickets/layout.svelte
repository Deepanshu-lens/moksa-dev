<script lang="ts">
  ...
  import { ChevronRight, Gift, Search, Ticket } from "lucide-svelte";
  ...
  const handleSubmitRequestFeature = async (event: any) => {
    ...
    try {
      const response = await fetch(
        `${import.meta.env.PUBLIC_MOKSA_BASE_URL}/requestfeature/create`,
        {
          method: "POST",
  ...
  const fetchNodes = async () => {
    ...
    let temp = await pb.collection("node").getFullList({
    ...
    temp = temp?.map((e) => ({
      ...
      expand: {
        camera: e.expand?.camera
          ? e.expand.camera.map((cam) => ({
              id: cam.id,
              name: cam.name,
              moksaId: cam.moksaId,
