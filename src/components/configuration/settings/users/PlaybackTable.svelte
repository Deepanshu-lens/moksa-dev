<script>
 ...
 function initializeUserFeatures() {
  ...
  <Table.Root class="mx-auto h-full w-full mt-1 flex flex-col max-w-[calc(100vw-8.3rem)] max-h-[calc(100vh-310px)] hide-scrollbar overflow-x-auto">
    <Table.Header
    ...
    <Table.Body class=" pb-10">
      ...
      >
          ...
          {#each playbackFeatures as feature,index}
            <Table.Cell class={`text-[#727272] h-full w-[180px] ${index !== playbackFeatures.length - 1 ? 'border-r' : ''}`}>
              <input type="checkbox" 
      ...
      {/each}
        ...
        <Button class='mr-auto bg-[#3D81FC] text-white' on:click={handleFeaturesUpdate}>
      save
    </Button>
  </Table.Root>
