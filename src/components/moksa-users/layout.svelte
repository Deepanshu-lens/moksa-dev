<script lang="ts">
  ...
  import UsersDataTable from "@/components/moksa-users/UsersDataTable.svelte";
  ...
  $: {
    if ($moksaUsers) {
      userData.set($moksaUsers?.users);
      userRole.set($moksaUsers?.userRole);
    }
  }
  ...
  const handleSubmit = async (
    ...
    try {
      ...
      if (allUsers.ok) {
        userData.set((await allUsers.json()).data?.data);
      }
      ...
      <section class="w-full p-4 px-6">
        <div class="flex items-center justify-end">
          <span class="flex items-center gap-3">
            ...
            <AddUserDialog {moksa} {handleSubmit} {userRole}>
              <Button
