<script lang="ts">
  ...
  $: percentageActive =
    totalCount === 0
      ? 0
      : Math.round((parseInt(activeCount) / parseInt(totalCount)) * 100);
...
<div
  class="flex items-center justify-between w-full p-4 h-[75px]"
  style={$mode !== "dark" ? `background-color: ${bgColor};` : ""}
>
  ...
  <div class="flex-col items-center justify-center flex gap-1 w-1/3">
    ...
    <span class=" text-[#323232] font-semibold text-xs"
      >{managerName.length > 12
        ? managerName.slice(0, 12) + "..."
        : managerName}</span
    >
  </div>
  ...
  <div class="circle-progress ">
    <svg viewBox="0 0 36 36">
      ...
      <path
        ...
        stroke="#FFF900"
        stroke-width="3"
        stroke-dasharray="{(parseInt(activeCount) / parseInt(totalCount)) *
          100}, 100"
      />
    </svg>
    <span class="progress-text"
      >{parseInt(activeCount)}/{parseInt(totalCount) > 1000 ? `${Math.round(parseInt(totalCount) / 1000)}k` : parseInt(totalCount)}</span
    >
  </div>
  </div>
  <!-- </div> -->
